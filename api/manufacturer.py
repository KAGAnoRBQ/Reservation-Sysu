...
from api.form import AddManufacturerForm, DeleteByIdForm, QueryDefaultForm
...
def add_manufacturer():
    form = AddManufacturerForm(request.form)
    if not form.validate():
        return reply(success=False, message='参数错误', error_code=const.param_err)

    if utils.is_name_exist(Manufacturer, form.name.data)[0]:
...
def delete_manufacturer():
    ...
    res = utils.delete_by_id(Manufacturer, form.id.data)
    return reply(success=res[0], message=res[1], error_code=res[2])
